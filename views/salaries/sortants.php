        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <span class="badge" style="background:rgba(239,68,68,0.12); color:#f87171; border:1px solid rgba(239,68,68,0.35);"><?= htmlspecialchars($s['date_sortie'] ?? '—') ?></span>
                    </td>
                    <td><?= htmlspecialchars($s['motif_sortie'] ?? '—') ?></td>
                    <td>
                        <div class="table-actions">
                            ...
                            <form method="POST" action="/paie-me/salaries/<?= $s['id'] ?>/reintegrer" class="inline-form">
                                <?= \Core\Session::csrfField() ?>
                                <button type="submit" class="btn-icon btn-view" title="Réintégrer" onclick="return confirm('Réintégrer <?= htmlspecialchars($s['nom_famille'] . ' ' . $s['prenom'], ENT_QUOTES) ?> dans les salariés actifs ?')">
                            ...
                            <form method="POST" action="/paie-me/salaries/<?= $s['id'] ?>/delete" class="inline-form">
                                <?= \Core\Session::csrfField() ?>
                                <button type="submit" class="btn-icon btn-delete" title="Supprimer définitivement" onclick="return confirm('Supprimer définitivement <?= htmlspecialchars($s['nom_famille'] . ' ' . $s['prenom'], ENT_QUOTES) ?> ? Cette action est irréversible.')">
