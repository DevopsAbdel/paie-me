...
- **Listes déroulantes dans le modèle** : les colonnes `enum` (Sexe, Situation, Type de contrat, Type de salaire, Fréquence, Mode de paiement) et les `m2o` (Service, Fonction, Société) ont une **data validation Excel** (`DataValidation::TYPE_LIST`) qui **interdit toute valeur non conforme à la base**. Les valeurs sont écrites dans une feuille cachée « Listes », référencée par `setDataValidation('X2:X2000', ...)`. Sources : `allowed` + variantes accentuées (`labelMap`) pour les enums ; services/fonctions du référentiel (société active si contexte, sinon toutes) ; sociétés de l'utilisateur (active seule si contexte). Piège OOXML : `showDropDown` est **inversé** → appeler `setShowDropDown(true)` pour AFFICHER la flèche (false = masquée) ; utiliser `new DataValidation()` (pas `getDataValidation()`) pour éviter les doublons dans le XML.
